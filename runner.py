...
def run(G, D, n=None):
    ...
    while n is None or k < n:
        ...
        for i in range(iterations):
            G_inputs = np.random.uniform(0.0,1.0,size)

            for G_genome_id, G_genome in G_genomes:
                G_net = neat.nn.FeedForwardNetwork.create(G_genome, G.config)
                ind_G_outputs = []
                for xi in G_inputs:
                    output = G_net.activate([xi])
                    ind_G_outputs.append(output[0])

                real_inputs = np.random.normal(.7,.05,size)
                for D_genome_id, D_genome in D_genomes:
                    D_net = neat.nn.FeedForwardNetwork.create(D_genome, D.config)
                    #Generator Input
                    mu = np.mean(ind_G_outputs)
                    sigma = np.var(ind_G_outputs)
        ...
        #         #generator wins
        #         if fake_output[0] >= 0.5:
        #             tempfit += float(D_genome.fitness/float(avg_D_fitness))
        #         #discrimator wins
        #         else:
        #             tempfit -= float(float(avg_D_fitness)/D_genome.fitness)
        #         # 1/True = generator won 0/False = discriminator won 
        #         # [G-genome, D-genome]: old fitness of G, result]
        #         masterDict[(G_genome, D_genome)] = [G_genome.fitness, fake_output[0] >= 0.5]

        #     G_genome.fitness = tempfit

        # #RESCALE DISCRIMINATOR FITNESS
        # for D_genome_id, D_genome in D_genomes:

        #     tempfit = 0.0

        #     for G_genome_id, G_genome in G_genomes:
        #         G_fitness, result = masterDict[(G_genome, D_genome)]
        #         #Generator wins
        #         if result:
        #             tempfit -= float(float(avg_G_fitness)/G_fitness)
        #         #Discriminator wins
        #         else:
        #             tempfit += float(G_fitness/float(avg_G_fitness))

        #         real_inputs = np.random.normal(.7,.05,size)
        #         real_output = D_net.activate(real_inputs)
        #         #Discriminator detects a real number
        #         if real_output[0] >= 0.5:
        #             tempfit += 1000000000000
        #         else:
        #             tempfit -= 1000000000

        #     D_genome.fitness =  tempfit
        ...
        G_net = neat.nn.FeedForwardNetwork.create(bestG, G.config)
